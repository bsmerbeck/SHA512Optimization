	...
	case stage2 is
		...
		if (Opcode2=LDI) then
			...
			t1_val <= std_logic_vector(
							...
							unsigned(K_TABLE(to_integer(unsigned(register_file(to_integer(unsigned(RX2))))))) + unsigned( unsigned(register_file(to_integer(unsigned(RZ2))))& unsigned(register_file(to_integer(unsigned(RA2)))))
							));	
